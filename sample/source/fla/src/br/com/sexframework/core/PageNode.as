package br.com.sexframework.core
{
	...
	internal class PageNode implements IPageNode
	{
		...
		public function PageNode( info:XML, parentNode:PageNode )
		{
			...
			var len:uint = info.extra.length();
			for ( var j:uint = 0; j < len; j++ )
				vars[info.viriable[j].@name] = new VariableInfo(info.viriable[j].@name, info.viriable[j].@value);
		...
		private function prepareLoad():void
		{
			...
			var pagesToLoad:Array = breadCrumb.slice(breadCrumb.indexOf(this), breadCrumb.length);
			var loader:ISexLoader = info.loader || Sextant.defaultLoader;
		...
		{
			e.target.removeEventListener( TransitionEvent.HIDDEN, onHiddenLoaderToOpen );
			showPage();
		}
